...
document.getElementById("search").addEventListener("keyup", function (event) {
  document.querySelector("#containerCalculatebyWeaknesses").style.display =
    "none";
...
selectByType.addEventListener("change", function () {
  ...
  let countForType = searchPokemonByType(
    dataPokemon,
    selectByType.value
  ).length;
...
selectByWeaknesses.addEventListener("change", function () {
  ...
  showPokemons(
    "",
    searchPokemonByWeaknesses(dataPokemon, selectByWeaknesses.value)
  );
  let countForWeaknesses = searchPokemonByWeaknesses(
    dataPokemon,
    selectByWeaknesses.value
  ).length;
  countFilteredPokemon2.innerHTML = countForWeaknesses;
  let operationPercent = computeProperties(
    dataPokemon.length,
    countForWeaknesses
  );
...
pokemonContainer.addEventListener("click", function (e) {
  ...
  if (!target.className.startsWith("card")) {
    ...
    let selectedPokemon = searchPokemon(
      dataPokemon,
      ...
      <span class="close">&times;</span>
        <div class="containerByModal">
          ...
          <div class="information">
            ...
            <p class="generation-name"> Nombre: ${
            ...
            <div class="properties-pokemon">${weaknesses.join("")}</div><br><br>
            <div class="containerBtn"><button id="btnStats">Ver Stats</button></div>
          </div>
        </div>
      </div>`;
    }
    ...
    document.getElementById("btnStats").addEventListener("click", function () {
      ...
      let selectedPokemon2 = searchPokemon(
        dataPokemon,
        target.className.split(" ")[1]
      );
      for (let properties of selectedPokemon2) {
        ...
        new Chart(ctx, {
          type: "bar",
          data: {
            labels: ["Ataque", "Defensa", "Estamina", "Máximo-cp", "Máximo-hp"],
            datasets: [
              {
                label: "Stats of Pokemon",
                data: [
                  `${properties.stats["base-attack"]}`,
                  `${properties.stats["base-defense"]}`,
                  `${properties.stats["base-stamina"]}`,
                  `${properties.stats["max-cp"]}`,
                  `${properties.stats["max-hp"]}`,
                ],
                backgroundColor: [
                  "rgba(255, 99, 132, 0.2)",
                  "rgba(54, 162, 235, 0.2)",
